...
if char_root:
    ...
    if armature:
        ...
        # The RightArm looked good at (70, 0, -20). Let's fix LeftArm.
        # Mixamo rigs often mirror local axes. If X is forward, Z is twist...
        # Let's try (70, 0, 20) for right, and (70, 0, -20) for left? No, let's look at what we had:
        # Before: LeftArm(70, 0, 20) = horizontal. RightArm(70, 0, -20) = down.
        # Let's try LeftArm(70, 0, -20) - maybe the signs are just the same!
        rot("LeftArm", 70, 0, -20)
...
# 8. SHOTGUN
shotgun_obj = bpy.data.objects.get("Remington870")
if shotgun_obj:
    print(f"DEBUG: Found shotgun! Old Loc: {shotgun_obj.location}")
    # Sometimes Empties from glTF have weird transforms. Let's just apply it to the mesh directly if needed.
    # Actually, if we just place the empty at (0, 0, 0.77):
    ...
    if mesh:
        print(f"DEBUG: Shotgun mesh loc relative to parent: {mesh.location}")
...
# Only hide exactly the ones in base_names, NOT their clones!
