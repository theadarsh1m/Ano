...
export function DMMessageInput({ conversationId, recipientId }: DMMessageInputProps) {
  ...
  const handleUpload = async (file: File) => {
    ...
    setUploadProgress(0);
    try {
      const result = await uploadChatFile(file, (p) => setUploadProgress(p));
      return result;
    } finally {
  ...
  const handleTyping = () => {
    ...
    if (typingTimeoutRef.current) {
      ...
      socket.emit("dm_typing_start", { conversationId, nickname });
    }
    typingTimeoutRef.current = setTimeout(() => {
      socket.emit("dm_typing_stop", { conversationId, nickname });
        ...
        {selectedFile && (
          ...
          >
            <div className="flex items-center gap-3 bg-white/5 px-3 py-2 rounded-lg border border-white/10">
              ...
              <button onClick={clearSelectedFile} className="p-1.5 rounded-full hover:bg-white/10 text-gray-400 hover:text-white transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
          </motion.div>
