...
export class SlitherClientEngine {
  ...
  private isLocalDying = false;
  private deathShockwave: { x: number; y: number; radius: number; maxRadius: number; alpha: number } | null = null;
  ...
  // Multiplayer interpolation & client prediction
  private lastSnapshotTime = 0;
  private lastSentAngle: number | null = null;
  private lastAngleSentTime = 0;
  private lastSentBoostState: boolean | null = null;
  ...
  public start() {
    ...
    if (this.mode === 'SINGLEPLAYER') {
      ...
      if (!this.snakes.has(this.myUserId)) {
  ...
  private sendAngle() {
    if (this.mode !== 'SINGLEPLAYER' && this.socket && this.gameId) {
      ...
      if (
        ...
        now - this.lastAngleSentTime >= 33 ||
        Math.abs(this.targetAngle - this.lastSentAngle) > 0.03
      ) {
        this.lastSentAngle = this.targetAngle;
  ...
  private sendBoostState(boosting: boolean) {
    if (this.mode !== 'SINGLEPLAYER' && this.socket && this.gameId) {
      if (this.lastSentBoostState !== boosting) {
        this.lastSentBoostState = boosting;
        this.socket.emit('game_action', {
          gameId: this.gameId,
          userId: this.myUserId,
          action: 'boost_update',
  ...
  private moveSnakePhysics(snake: ClientSnake) {
    // Cannot boost if score/length is under 10
    ...
    // If boosting, continuously consume score & length, dropping food trail behind
    if (snake.isBoosting) {
      // Deduct score continuously
      snake.score = Math.max(10, snake.score - 0.20);

      // Drop food trail periodically
      if (Math.random() < 0.20 && snake.segments.length > 10) {
        ...
        this.foods.push({
          ...
          size: 5.5,
          color: this.getSkinColor(snake.skin),
          value: 4.0
        });
      }
    }

    // Ensure length matches score
  ...
  private killSnake(snake: ClientSnake) {
    ...
    // Explode segments into scatter food & glowing explosion particles
    for (let i = 0; i < snake.segments.length; i++) {
      ...
      if (i % 2 === 0) {
        this.foods.push({
          id: `food_${Math.random().toString(36).substr(2, 9)}`,
          x: seg.x + Math.cos(spreadAngle) * (Math.random() * 20 - 10),
          y: seg.y + Math.sin(spreadAngle) * (Math.random() * 20 - 10),
    ...
    // If player died, show food explosion animation and camera shockwave before GameOver modal
    if (snake.id === this.myUserId) {
      this.isLocalDying = true;
      this.deathShockwave = {
        x: head.x,
        y: head.y,
        radius: 10,
  ...
  private setupMultiplayer() {
    ...
    this.socket.on('game_state', (state: any) => {
      ...
      // Sync foods (combine server foods with active client-dropped foods)
      if (Array.isArray(state.foods)) {
        const serverFoods = state.foods.map((f: any) => ({
          id: f.id,
          x: f.x,
          y: f.y,
          size: f.size || 5,
          color: f.color || '#00FF66',
          value: f.value !== undefined ? f.value : 1,
          attractedTo: f.attractedTo || undefined,
          attractionProgress: f.attractedTo ? 0.5 : undefined
        }));

        const serverFoodIds = new Set(serverFoods.map((f: any) => f.id));
        const localDroppedFoods = this.foods.filter(f => f.id.startsWith('food_') && !serverFoodIds.has(f.id));
        this.foods = [...serverFoods, ...localDroppedFoods];
      }
      ...
      if (Array.isArray(state.snakes)) {
        ...
        state.snakes.forEach((s: any) => {
          ...
          if (existing) {
            ...
            if (s.id === this.myUserId) {
              // Reconcile high drift for local player head position (>120px)
              if (s.segments && s.segments.length > 0 && existing.segments.length > 0) {
                const dx = s.segments[0].x - existing.segments[0].x;
                const dy = s.segments[0].y - existing.segments[0].y;
                if (dx * dx + dy * dy > 120 * 120) {
                  existing.segments[0].x = s.segments[0].x;
                  existing.segments[0].y = s.segments[0].y;
                }
              }
            } else {
              existing.isBoosting = s.isBoosting;
              existing.targetAngle = s.angle;
              existing.targetSegments = Array.isArray(s.segments) ? s.segments.map((seg: any) => ({ x: seg.x, y: seg.y })) : [];
            }
          } else {
            // New snake joined the screen
            const initialSegs = Array.isArray(s.segments) ? s.segments.map((seg: any) => ({ x: seg.x, y: seg.y })) : [];
            this.snakes.set(s.id, {
              ...
              targetSegments: initialSegs,
              width: 22 + Math.min(38, s.score * 0.1)
            });
          }
        });

        // Remove snakes no longer in the snapshot (only if non-empty snapshot)
  ...
  private updateMultiplayer() {
    // 1. Instant client-side physics prediction for local player (silky smooth, like singleplayer!)
    const player = this.snakes.get(this.myUserId);
    if (player && player.isAlive) {
      player.isBoosting = this.isBoosting && player.segments.length > 10;
    ...
    // 2. Interpolate remote snakes (opponents & bots)
  ...
  private render() {
    ...
    // 3.5. Draw death particles and shockwave
  ...
  private drawDeathParticles() {
    ...
    // Draw expanding shockwave ring on local player death
    if (this.deathShockwave) {
      const sw = this.deathShockwave;
      sw.radius += (sw.maxRadius - sw.radius) * 0.08;
      sw.alpha -= 0.018;

      if (sw.alpha > 0) {
        this.ctx.save();
        this.ctx.strokeStyle = `rgba(255, 0, 85, ${Math.max(0, sw.alpha)})`;
