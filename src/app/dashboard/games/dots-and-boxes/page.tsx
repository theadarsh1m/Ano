...
function DotsAndBoxesPageContent() {
  ...
  if (lobby && !gameState) {
    ...
    return (
      <div className="flex flex-col h-screen bg-black text-white">
        <div className="flex flex-wrap items-center justify-between p-3 sm:p-4 bg-white/5 border-b border-white/10 gap-2">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <button 
              onClick={() => setShowRulesModal(true)}
              className="px-2.5 sm:px-4 py-2 bg-white/5 border border-white/10 text-gray-300 hover:text-white rounded-full text-xs sm:text-sm font-bold flex items-center gap-1.5 transition-colors hover:bg-white/10 animate-pulse cursor-pointer"
            >
              <BookOpen className="w-4 h-4" /> <span className="hidden sm:inline">Rules</span>
            </button>
            <button 
              onClick={() => setShowInviteModal(true)} 
              className="px-2.5 sm:px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-full text-xs sm:text-sm font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
            >
              <UserPlus className="w-4 h-4" /> <span className="hidden sm:inline">Invite</span>
            </button>
            <button 
              onClick={handleLeave} 
              className="px-2.5 sm:px-4 py-2 bg-red-600/20 hover:bg-red-600/40 border border-red-500/30 rounded-full text-xs sm:text-sm font-bold text-red-400 flex items-center gap-1.5 transition-colors cursor-pointer"
            >
                    ...
                    <div key={user.id} className="flex items-center justify-between p-3 bg-white/5 rounded-xl mb-2">
                      ...
                      <button
                        ...
                        className={`px-3 py-1 rounded-full text-xs font-bold transition-colors ${
                          !status.canInvite ? 'bg-blue-600/30 text-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-500 cursor-pointer'
                        }`}
                      >
                        {status.label}
                      </button>
