...
import { LogOut, Lock, Unlock, Copy, Check, Wifi, WifiOff } from "lucide-react";
...
export default function RoomPage() {
  ...
  const { rooms, leaveRoom: storeLeaveRoom } = useRoomStore();
  ...
  const room = isClient ? rooms.find(r => r.id === roomId || r.id === roomId.toUpperCase()) : null;
  ...
  useEffect(() => {
    ...
    if (isClient && room && userId && nickname) {
      ...
      return () => {
        ...
        clearChat(roomId); // Clear when leaving
      };
    }
  }, [isClient, room, userId, nickname, roomId, setConnectionStatus, addMessage, addTypingUser, removeTypingUser, setActiveUsers, clearChat, router]);
        ...
        <div>
          ...
          <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground">
            {isPrivate && (
              ...
              >
                Code: <span className="font-mono tracking-wider">{room.id}</span>
